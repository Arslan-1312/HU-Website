...
import { meritAlert } from '../data/admissionData';

const MeritAlert = () => {
    const [isOpen, setIsOpen] = useState(false);

    useEffect(() => {
        if (meritAlert?.enabled) {
            setIsOpen(true);
        }
    }, []);

    if (!isOpen || !meritAlert) return null;
            ...
            <div className="relative bg-white rounded-2xl shadow-2xl border-4 border-custom-green max-w-lg w-full p-8 transform transition-all animate-in zoom-in-95 duration-300">
                ...
                <div className="space-y-6">
                    ...
                    <div className="space-y-4">
                        {meritAlert.links.map((link, index) => (
                            <Link
                                key={index}
                                to={link.path}
                                onClick={() => setIsOpen(false)}
